function[fullNewTonAns] = fullNewtonFiniteElementMethodLinearVerification(initGuess, lambda, tolerence, gridSize)
    h = 1 / (gridSize - 1);
    h2Inv = 1 / (h * h);
    % lambda = 1;
        ...
        for row = 1:gridSize
            for col = 1:gridSize
                ...
                if atBorder(idx, gridSize)
                    ...
                    UiAddNx = U(idxAddNx);
                    UiMinNx = U(idxMinNx);
                    R(idx) = -((UiAdd1 - 2 * Ui + UiMin1) * h2Inv + (UiAddNx - 2 * Ui + UiMinNx) * h2Inv + 2);
                end                
            end
        end
        % solve deltaU:
